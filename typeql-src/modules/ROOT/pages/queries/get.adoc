...
:pageTitle: Get query
:page-aliases: {page-component-version}@typeql::data/get.adoc

A Get query retrieves concepts as stateful objects from a TypeDB database that you can further process with the
xref:{page-component-version}@drivers::overview.adoc#_driver_api[driver API] methods.
For a practical guide on how to send a Get query to TypeDB,
see the xref:{page-component-version}@manual::reading/get.adoc[] page of the TypeDB Manual.
For response processing examples, see the xref:{page-component-version}@manual::objects/data.adoc[data objects] and
xref:{page-component-version}@manual::objects/schema.adoc[schema objects] pages.
...
A Get query retrieves xref:{page-component-version}@typeql::concepts/overview.adoc[concepts] from a database.
...
xref:{page-component-version}@typeql::modifiers/aggregation.adoc[aggregation], xref:{page-component-version}@typeql::modifiers/grouping.adoc[grouping], or both.
See the <<_response>> section below.

You can use xref:{page-component-version}@typeql::modifiers/sorting.adoc[sorting] and
xref:{page-component-version}@typeql::modifiers/pagination.adoc[pagination] with a Get query.

Get queries can use rule-based xref:{page-component-version}@manual::reading/infer.adoc[inference].
...
optionally processed by xref:{page-component-version}@typeql::modifiers/overview.adoc[modifiers].
...
see the xref:{page-component-version}@manual::objects/schema.adoc[] and xref:{page-component-version}@manual::objects/data.adoc[] pages.
...
Applying xref:{page-component-version}@typeql::modifiers/overview.adoc[modifiers] can have a drastic effect on the format and number
of results returned by a Get query.
See the <<_table_output_types,table above>> for output formats depending on the usage of
xref:{page-component-version}@typeql::modifiers/grouping.adoc[grouping]
and xref:{page-component-version}@typeql::modifiers/aggregation.adoc[aggregation].

Usage of the xref:{page-component-version}@typeql::modifiers/pagination.adoc#_limit_the_results[limit] modifier directly sets a limitation
for the number of results.

Combination of <<_get_clause_filtering>> and xref:{page-component-version}@typeql::modifiers/sorting.adoc[] can produce
...
The xref:{page-component-version}@typeql::modifiers/aggregation.adoc#_count[count modifier] is added to get the number of results,
...
xref:{page-component-version}@academy::7-understanding-query-patterns/7.6-solution-set-semantics.adoc[] page in TypeDB Academy.
////
...
[%collapsible]
====
In xref:{page-component-version}@manual::studio.adoc[TypeDB Studio], we can paste the following TypeQL in the text editor
...
To insert this dataset with any xref:{page-component-version}@drivers::overview.adoc[other] client --
make sure to send every line, except from comments, as a separate insert query.

For more information on an Insert query, see the xref:{page-component-version}@typeql::queries/insert.adoc[] page.
====
