...
:page-aliases: {page-component-version}@typeql::data/insert.adoc
...
see the xref:{page-component-version}@manual::writing/insert.adoc[] page of the TypeDB Manual.
...
----
// end::syntax[]
where `<insert_pattern>` is a pattern of valid data-specific TypeQL statements including xref:{page-component-version}@typeql::statements/isa.adoc[],
xref:{page-component-version}@typeql::statements/has.adoc[],
xref:{page-component-version}@typeql::statements/role-assignment.adoc[role assignment], and xref:{page-component-version}@typeql::statements/value-assignment.adoc[value assignment].

The _optional_ `match` clause `<pattern>` can be any valid match pattern: for more information, see the
xref:{page-component-version}@typeql::patterns/overview.adoc[Patterns] and xref:{page-component-version}@typeql::statements/overview.adoc[Statements] sections.
...
xref:{page-component-version}@typeql::statements/isa.adoc[isa] statements.
* Ownerships of attributes are inserted using a xref:{page-component-version}@typeql::statements/has.adoc[has] statement. Note that inserting an ownership of an attribute that doesn't yet exist in a database (with a new value), implicitly inserts such an attribute.
* Role players are usually assigned as a part of a <<_inserting_relations,relation insertion>> with a
xref:{page-component-version}@typeql::statements/role-assignment.adoc[role assignment] statement.
// end::insert_clause[]
...
To insert an entity, use a concept variable followed by an xref:{page-component-version}@typeql::statements/isa.adoc[isa]
...
or combine statements into a xref:{page-component-version}@typeql::statements/overview.adoc#_composite_statements[composite statement].
...
You can also use the xref:{page-component-version}@typeql::statements/key.adoc[@key] or
xref:{page-component-version}@typeql::statements/unique.adoc[@unique] annotations for an attribute type ownership schema definition
...
You can specify the value with a xref:{page-component-version}@typeql::values/comparators.adoc[comparator] or
xref:{page-component-version}@typeql::statements/value-assignment.adoc[] statement.
...
xref:{page-component-version}@typeql::queries/delete.adoc#_orphaned_attributes[orphaned attributes]
and aren't very useful to store in a database.

To implicitly insert an attribute, assign an ownership of it with a xref:{page-component-version}@typeql::statements/has.adoc[has] statement.
...
To insert an ownership of an attribute, use a xref:{page-component-version}@typeql::statements/has.adoc[] statement:
...
//
// image::{page-component-version}@typeql::john-6-attributes.png[John with 4 emails, role=framed, width = 75%, link=self]
...
Use a xref:{page-component-version}@typeql::statements/role-assignment.adoc[] statement with an
xref:{page-component-version}@typeql::statements/isa.adoc[isa] statement in an `insert clause` to insert a relation.
...
match the relation from a database and use a xref:{page-component-version}@typeql::statements/role-assignment.adoc[] statement
with only new roleplayers.
Don't use an xref:{page-component-version}@typeql::statements/isa.adoc[] statement with relation type
